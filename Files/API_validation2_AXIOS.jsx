...
function App() {
  const [data, setData] = useState({ firstname: "", lastname: "", date: "", email: "", mobile:"" });
  const [submit, setSubmit] = useState([]);
  const [editIndex, setEditIndex] = useState(null);
  const [validationErrors, setValidationErrors] = useState({});

  const token = "C37vggOOoqInKOet";

  const getData = () => {
    axios.get("https://generateapi.techsnack.online/api/project2", {
      headers: {
        Authorization: token,
      },
    })
  ...
  const postData = () => {
    ...
    axios.post("https://generateapi.techsnack.online/api/project2", formData,{      
      headers: {
        Authorization: token,
      },
    })
    .then((response) => {
      ...
      if (response.data.Status === "Success") {
        getData();
        setData({ firstname: "", lastname: "", date:"", email: "", mobile:"" });
  ...
  const deleteData = (index, id) => {
    axios.delete(`https://generateapi.techsnack.online/api/project2/${id}`, {
      headers: {
        Authorization: token,
      },
    })
  ...
  const updateData = (id) => {
    ...
    axios.patch(`https://generateapi.techsnack.online/api/project2/${id}`, formData, {
      headers: {
        Authorization: token,
      },
    })
  ...
  return (
    <div>
      ...
      />
      <br />
      {validationErrors.date && <div style={{ color: "red" }}>{validationErrors.date}</div>}
      <br />
      ...
      />
      <br />
      {validationErrors.mobile && <div style={{ color: "red" }}>{validationErrors.mobile}</div>}
      <br />
